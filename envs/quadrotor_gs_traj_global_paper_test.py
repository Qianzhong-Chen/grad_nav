...
class QuadrotorGSMaskPosTrajGlobalPaperTestEnv(DFlexEnv):
    ...
    def __init__(self, 
        ...
        num_act = 4
        self.agent_name = 'quadrotor_gs_mask_pos_traj_global_paper_test'
        self.num_history = HISTORY_BUFFER_NUM
        self.num_latent = LATENT_VECT_NUM
    
        print('----------------------------', device)
        super(QuadrotorGSMaskPosTrajGlobalPaperTestEnv, self).__init__(num_envs, num_obs, num_act, episode_length, MM_caching_frequency, seed, no_grad, render, device)
        ...
        self.gs_init_pos = torch.tensor([[-6.0, -0., -0.2]] * self.num_envs, device=self.device) # (x,y,z); gs dimension for visual info
        self.point_could_init_pos = torch.tensor([[-6.0, -0., -0.2]] * self.num_envs, device=self.device) # (x,y,z); point cloud dimension for obstacle distance & traj plan
