...
class QuadrotorSimulator:
    def __init__(self, mass=1.0, inertia=None, link_length=0.1, Kp=None, Kd=None, 
                 freq=100.0, max_thrust=15.0, total_time=1.0, rotor_noise_std=None):
        """
    ...
    def update(self, position, velocity, orientation, angular_velocity, omega_desired, thrust):
        ...
        if self.rotor_noise_std is not None:
            # omega_noise = torch.randn_like(omega_desired) * self.rotor_noise_std
        ...
        omega_cross = torch.cross(angular_velocity, torch.bmm(I, angular_velocity.unsqueeze(2)).squeeze(2))
