...
class QuadrotorGSMaskPosTrajGlobalPPOEnv(DFlexEnv):
    ...
    def init_sim(self):
        ...
        self.max_thrust = torch.full((self.num_envs,), 25.0, device=self.device)  # All drones have max_thrust = 23 N
    ...
    def reset(self, env_ids = None, force_reset = True):
        ...
        if env_ids is not None:
            if not self.visualize and self.domain_randomization:
                ...
                mass_noise = torch.rand(num_reset_envs, device=self.device) * 0.4 + 0.8  # Uniform noise between 0.8 and 1.2
                self.mass[env_ids] = mass_noise
                max_thrust_noise = torch.rand(num_reset_envs, device=self.device) * 10.0 + 20.0  # Uniform noise between 20 and 30
    ...
    def calculateReward_archive(self):
        # torso_pos = self.state_joint_q.view(self.num_envs, -1)[:, 0:3]
