...
class QuadrotorGSMaskPosTrajGlobal8Env(DFlexEnv):
    ...
    def __init__(self, 
        ...
        num_act = 4
        self.agent_name = 'quadrotor_gs_mask_pos_traj_global_8'
        self.num_history = HISTORY_BUFFER_NUM
        self.num_latent = LATENT_VECT_NUM
    
        print('----------------------------', device)
        super(QuadrotorGSMaskPosTrajGlobal8Env, self).__init__(num_envs, num_obs, num_act, episode_length, MM_caching_frequency, seed, no_grad, render, device)
        ...
        self.point_could_init_pos = torch.tensor([[-6.0, -0., 0.0]] * self.num_envs, device=self.device) # (x,y,z); point cloud dimension for obstacle distance & traj plan
    ...
    def init_sim(self):
        ...
        # inertia = torch.tensor([0.008, 0.01, 0.02], device=self.device)  # Diagonal inertia components
        ...
        # Kp = torch.tensor([0.8, 1.0, 2.0], device=self.device).unsqueeze(0).repeat(self.num_envs, 1) 
    ...
    def reset(self, env_ids = None, force_reset = True):
        ...
        if env_ids is not None:
            if not self.visualize and self.domain_randomization:
                ...
                # inertia = torch.tensor([0.008, 0.01, 0.02], device=self.device)  # Base inertia
