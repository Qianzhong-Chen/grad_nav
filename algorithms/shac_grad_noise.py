...
class SHAC:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        act_grad = torch.zeros_like(actions)
        obs_grad = torch.zeros_like(obs)
        ...
        for i in range(self.steps_num):
            ...
            # # add dL/dact, gradients of actor_loss w.r.t. actions to actions
            # act_grad = (0.1 ** 0.5) * act_grad
            # actions += act_grad

            obs, rew, done, extra_info = self.env.step(torch.tanh(actions))
            
            # add dL/dobs, gradients of actor_loss w.r.t. obs to obs
            obs_grad = (0.1 ** 0.5) * obs_grad
            obs += obs_grad
            ...
            # # calculate dL/dact, gradient of actor_loss w.r.t actions
            # act_grad = torch.autograd.grad(actor_loss, actions, retain_graph=True)[0]
            # act_grad = normalize(act_grad, p=2.0, dim=1)

            # calculate dL/dobs, gradient of actor_loss w.r.t obs
            obs_grad = torch.autograd.grad(actor_loss, obs, retain_graph=True)[0]
            obs_grad = normalize(obs_grad, p=2.0, dim=1)
