...
class SHAC7:
    def __init__(self, cfg):
        ...
        self.visual_net_finetune = cfg['params']['config'].get('visual_net_fintune', False)
        if self.visual_net_finetune:
            self.visual_net_lr = float(cfg['params']['config'].get('visual_net_learning_rate', 1e-4))
        # lr schedule
        ...
        if self.visual_net_finetune:
            self.visual_net_optimizer = torch.optim.AdamW(self.env.visual_net.fc.parameters(), betas = cfg['params']['config']['betas'], lr = self.visual_net_lr, weight_decay=1e-2)
        # replay buffer
    ...
    def train(self):
        ...
        def actor_closure():
            ...
            self.visual_net_optimizer.zero_grad()
            self.time_report.start_timer("backward simulation")
            actor_loss.backward()
            self.time_report.end_timer("backward simulation")
            self.visual_net_optimizer.step()
        ...
        #     wandb.init(
        #         # set the wandb project where this run will be logged
        #         project=f'{self.env.map_name}-{self.env.agent_name}-{self.map_name}',
        #         # track hyperparameters and run metadata
        #         config=self.hyper_parameter
        #     )
        # else:
