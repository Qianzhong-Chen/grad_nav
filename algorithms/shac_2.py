...
class SHAC2:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
            ...
            obs, privilege_obs, rew, done, extra_info = self.env.step(torch.tanh(actions))
    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            # learning rate schedule
                ...
                actor_lr = self.actor_lr * 0.5 * (1 + math.cos(math.pi * epoch / self.max_epochs))
                for param_group in self.actor_optimizer.param_groups:
                    param_group['lr'] = actor_lr
                lr = actor_lr

                critic_lr = self.critic_lr * 0.5 * (1 + math.cos(math.pi * epoch / self.max_epochs))
                for param_group in self.critic_optimizer.param_groups:
                    param_group['lr'] = critic_lr
