...
class VAE(nn.Module):
    def __init__(self, 
        ...
        self.height_mu = nn.Linear(num_latent * 4, 1)
        self.height_var = nn.Linear(num_latent * 4, 1)
    ...
    def encode(self,obs_history):
        ...
        return [latent_mu, latent_var, vel_mu, vel_var, height_mu, height_var]

    def decode(self,z,v,h):
        input = torch.cat([z,v,h], dim = 1)
        output = self.decoder(input)
        return output

    def forward(self,obs_history):
        latent_mu, latent_var, vel_mu, vel_var, height_mu, height_var = self.encode(obs_history)
        ...
        return [z, vel, height],[latent_mu, latent_var, vel_mu, vel_var, height_mu, height_var]
    ...
    def loss_fn(self,obs_history, next_obs, vel, height, kld_weight = 1.0, velo_weight = 1.0, height_weight = 1.0):
        estimation, latent_params = self.forward(obs_history)
        z, v, h = estimation
        latent_mu, latent_var, vel_mu, vel_var, height_mu, height_var= latent_params 
        latent_mu = torch.clamp(latent_mu, min=-10, max=10)
        latent_var = torch.clamp(latent_var, min=-10, max=10)

        # Reconstruction loss
        recons = self.decode(z,vel,height)
        ...
        loss = recons_loss + velo_weight * vel_loss + height_weight * height_loss + kld_weight * kld_loss
    ...
    def inference(self,obs_history):
        ...
        latent_mu, latent_var, vel_mu, vel_var, height_mu, height_var = latent_params
        return [latent_mu, vel_mu, height_mu]
