...
# VAE net also estimates velocity and height, 
# Also training a seperate state-estimate net
...
from .vae_4 import VAE
from .velo_net_2 import VELO_NET
...
class SHAC6:
    def __init__(self, cfg):
        ...
        if cfg['params']['general']['checkpoint'] != 'Base':
            ...
            print_info(f'[INFO] actor critic networks recovered from {ckpt_path}')
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
            ...
            obs, privilege_obs, history_obs, vel_obs, height_obs, rew, done, extra_info = self.env.step(torch.tanh(actions), vae_output, vel_net_output)
            self.obs_hist_buf = history_obs

            

            # update VAE
            recons_loss, kld_loss = self.vae_closure(obs, vel_obs, height_obs, history_obs, done)
            vel_loss = self.vel_net_closure(obs, vel_obs, height_obs, history_obs, done)
    ...
    def vae_closure(self, obs, vel_obs, height_obs, history_obs, done):
        self.vae_optimizer.zero_grad()
        vae_loss_dict = self.vae.loss_fn(history_obs.clone().detach(), obs.clone().detach(), vel_obs.clone().detach(), height_obs.clone().detach(), self.kl_weight, self.velo_weight)
    ...
    def vel_net_closure(self, obs, vel_obs, height_obs, history_obs, done):
        self.vel_net_optimizer.zero_grad()
        raw_loss = self.vel_net.loss_fn(history_obs.clone().detach(), vel_obs.clone().detach(), height_obs.clone().detach())
        valid = (done == 0).squeeze()
    ...
    def train(self):
        ...
        if self.pretrain_vel_net:
            print_info('[INFO] pretraining VEL_NET with large roll')
            self.env.training_stage = 0 # set obs to vel_net training status
        ...
        self.env.training_stage = 1 # set obs to initial training status
        for epoch in range(self.max_epochs):
            ...
            if self.multi_stage:
                if epoch == self.stage_change_time:
                    ...
                    print_info(f'[INFO] change training setting, using obs of training stage {self.env.training_stage}')

            if epoch == self.vel_net_stop_time:
                print_info(f'[INFO] vel_net training paused')
            ...
            if self.lr_schedule == 'linear':
                ...
                if self.env.training_stage == 1:
                    ...
                    if self.vel_net_early_stop and (epoch > self.vel_net_stop_time):
                        vel_net_lr = 0.0
                if self.env.training_stage == 2:
                    vel_net_lr = (1e-5 - self.vel_net_lr) * float(abs(epoch - self.stage_change_time) / self.max_epochs) + self.vel_net_lr
            ...
            # learning rate schedule
                ...
                if self.env.training_stage == 1:
                    ...
                    if self.vel_net_early_stop and (epoch > self.vel_net_stop_time):
                        vel_net_lr = 0.0
                if self.env.training_stage == 2:
                    vel_net_lr = 1e-5 + (self.vel_net_lr - 1e-5) * 0.5 * (1 + math.cos(math.pi * abs(epoch - self.stage_change_time) / self.max_epochs))
                for param_group in self.vel_net_optimizer.param_groups:
                    param_group['lr'] = vel_net_lr
            else:
                lr = self.actor_lr
    ...
    def load(self, path):
        ...
        print_info(f"[INFO] all nets have been loaded")
    ...
    #     if os.path.exists(os.path.join(ckpt_dir, 'visual_net.pth')):
    #         self.load_visual_net(os.path.join(ckpt_dir, 'visual_net.pth'))
    #     if os.path.exists(os.path.join(ckpt_dir, 'best_vae.pth')):
    #         self.load_vae(os.path.join(ckpt_dir, 'best_vae.pth'))
    #     if os.path.exists(os.path.join(ckpt_dir, 'best_vel_net.pth')):
    #         self.load_vel_net(os.path.join(ckpt_dir, 'best_vel_net.pth'))
    #     self.run(cfg['params']['config']['player']['games_num'])
    #     # temporary save deploy policy
    #     # self.save_temporary(cfg, 'deploy_policy')
    ...
    # def save_visual_net(self):
    #     if self.env.visual_net is not None:
    #         torch.save(self.env.visual_net.state_dict(), os.path.join(self.log_dir, 'visual_net.pth'))

    # def save_vae(self, filename=None):
    #     if self.vae is not None:
    #         if filename is None:
    #             filename = 'best_vae'
    #             torch.save(self.vae.state_dict(), os.path.join(self.log_dir, f'{filename}.pth'))

    # def save_vel_net(self, filename=None):
    #     if self.vel_net is not None:
    #         if filename is None:
    #             filename = 'best_vel_net'
    #             torch.save(self.vel_net.state_dict(), os.path.join(self.log_dir, f'{filename}.pth'))
    ...
    #     # self.vae = checkpoint[4].to(self.device)
    #     # self.ret_rms = checkpoint[5].to(self.device) if checkpoint[5] is not None else checkpoint[5]

    #     self.ret_rms = checkpoint[4].to(self.device) if checkpoint[4] is not None else checkpoint[4]

    # def load_visual_net(self, path):
    #     self.env.visual_net.load_state_dict(torch.load(path))
    #     self.env.visual_net.eval()
    #     print_info(f"visual net parameters loaded")

    # def load_vae(self, path):
    #     self.vae.load_state_dict(torch.load(path))
    #     self.vae.eval()
    #     print_info(f"vae net parameters loaded")

    # def load_vel_net(self, path):
    #     self.vel_net.load_state_dict(torch.load(path))
    #     self.vel_net.eval()
    #     print_info(f"vel_net parameters loaded")
