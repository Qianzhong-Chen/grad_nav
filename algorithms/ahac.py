...
class AHAC:
    def __init__(self, cfg):
        ...
        # AHAC special
        self.steps_min = int(cfg["params"]["config"]["steps_min"])  # minimum horizon
        self.steps_max = int(cfg["params"]["config"]["steps_max"])  # maximum horizon
        self.contact_threshold= cfg["params"]["config"]["contact_threshold"]  # for cutting horizons
        self.accumulate_jacobians = cfg["params"]["config"].get("accumulate_jacobians", False)  # if true clip gradients by accumulation
        self.eval_runs = cfg["params"]["config"].get("eval_runs", 12)
        self.lambd_lr = float(cfg['params']['config']['lambd_lr'])
        self.critic_iterations = None  # if None, we do early stop
        if cfg['params']['config']['critic_iterations']:
            self.critic_iterations = cfg['params']['config']['critic_iterations']
        self.H = torch.tensor(self.steps_min, dtype=torch.float32, device=self.device)
        self.lambd = torch.tensor([0.0]*self.steps_min, dtype=torch.float32, device=self.device)
        self.C = self.contact_threshold
        self.critic_batches = cfg["params"]["config"]["critic_batches"]
        self.critic_batch_size = self.num_envs * self.steps_max // self.critic_batches
        ...
        if cfg['params']['general']['checkpoint'] is not 'Base':
        ...
        self.acc_jacobians = self.accumulate_jacobians
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
            ...
            term = extra_info["termination"]
            trunc = extra_info["truncation"]
            ...
            # TODO: contact truncation not implemented
            ...
            if i < self.steps_num - 1:
                retrn = (- rew_acc[i + 1, done_env_ids] - self.gamma * gamma[done_env_ids] * next_values[i + 1, done_env_ids])
                actor_loss = actor_loss + retrn.sum()
                with torch.no_grad():
    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            # pdb.set_trace()
            print('iter {}: ep loss {:.2f}, ep discounted loss {:.2f}, ep len {:.1f}, H {:.2f}, fps total {:.2f}, value loss {:.2f}, grad norm before clip {:.2f}, grad norm after clip {:.2f}'.format(\
                    self.iter_count, mean_policy_loss, mean_policy_discounted_loss, mean_episode_length, self.H.item(), fps, self.value_loss, self.grad_norm_before_clip, self.grad_norm_after_clip))
            ...
            wandb.log({"loss": wandb_record_loss, 
    ...
    def save(self, filename = None):
        ...
        torch.save([self.actor, self.critic, self.obs_rms, self.ret_rms], os.path.join(self.log_dir, "{}.pt".format(filename)))
    ...
    def load(self, path):
        ...
        self.ret_rms = checkpoint[3].to(self.device) if checkpoint[3] is not None else checkpoint[3]
    ...
    def init_buffers(self):
        ...
        self.cfs = torch.zeros((self.steps_num, self.num_envs), dtype=torch.float32, device=self.device)
