...
from .vae_2 import VAE # new multi-head VAE can also estimate pos
...
class SHAC4:
    def __init__(self, cfg):
        ...
        self.pos_weight = cfg["params"]["vae"].get("pos_weight", 1.0)
        ...
        self.actor_optimizer = torch.optim.AdamW(self.actor.parameters(), betas = cfg['params']['config']['betas'], lr = self.actor_lr, weight_decay=1e-2)
        self.critic_optimizer = torch.optim.AdamW(self.critic.parameters(), betas = cfg['params']['config']['betas'], lr = self.critic_lr, weight_decay=1e-2)
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
            ...
            obs, privilege_obs, history_obs, vel_obs, pos_obs, rew, done, extra_info = self.env.step(torch.tanh(actions), vae_output)
            self.obs_hist_buf = history_obs

            # update VAE
            recons_loss, vel_loss, pos_loss, kld_loss = self.vae_closure(obs, vel_obs, pos_obs, history_obs, done)
            mean_recons_loss += recons_loss
            mean_pos_loss += pos_loss
        ...
        self.mean_pos_loss = mean_pos_loss
        self.mean_kld_loss = mean_kld_loss
        self.vae_loss = mean_recons_loss + self.velo_weight * mean_vel_loss + self.pos_weight * mean_pos_loss + self.kl_weight * mean_kld_loss
    ...
    def vae_closure(self, obs, vel_obs, pos_obs, history_obs, done):
        self.vae_optimizer.zero_grad()
        vae_loss_dict = self.vae.loss_fn(history_obs.clone().detach(), obs.clone().detach(), vel_obs.clone().detach(), pos_obs.clone().detach(), self.kl_weight, self.velo_weight, self.pos_weight)
        ...
        with torch.no_grad():
            ...
            pos_loss = torch.mean(vae_loss_dict['pos_loss'][valid])
            kld_loss = torch.mean(vae_loss_dict['kld_loss'][valid]) 
        return recons_loss.item(), vel_loss.item(), pos_loss.item(), kld_loss.item()
    ...
    def evaluate_policy(self, num_games, deterministic = False):
        ...
        while games_cnt < num_games:
            ...
            obs, privilege_obs, history_obs, vel_obs, pos_obs, rew, done, extra_info = self.env.step(torch.tanh(actions), vae_output)
            self.obs_hist_buf = history_obs
            # obs, _, rew, done, _ = self.env.step(torch.tanh(actions))
    ...
    def train(self):
        ...
        if hasattr(self.env, 'map_name'):
            wandb.init(
                # set the wandb project where this run will be logged
                project=f'{self.env.map_name}-{self.env.agent_name}',
        ...
        for epoch in range(self.max_epochs):
            ...
            if self.multi_stage:
                if epoch == 300:
                    self.env.training_stage = 1
                    print_info('change to use vae velo')
