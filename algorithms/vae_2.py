...
class VAE(nn.Module):
    def __init__(self, 
        ...
        self.pos_mu = nn.Linear(num_latent * 4, 3)
        self.pos_var = nn.Linear(num_latent * 4, 3)
    ...
    def encode(self,obs_history):
        ...
        pos_mu = self.pos_mu(encoded)
        pos_var = self.pos_var(encoded)
        return [latent_mu, latent_var, vel_mu, vel_var, pos_mu, pos_var]
    ...
    def forward(self,obs_history):
        latent_mu, latent_var, vel_mu, vel_var, pos_mu, pos_var = self.encode(obs_history)
        z = self.reparameterize(latent_mu, latent_var)
        vel = self.reparameterize(vel_mu, vel_var)
        pos = self.reparameterize(pos_mu, pos_var)
        return [z, vel, pos], [latent_mu, latent_var, vel_mu, vel_var, pos_mu, pos_var]
    ...
    def loss_fn(self,obs_history, next_obs, vel, pos, kld_weight = 1.0, velo_weight = 1.0, pos_weight=1.0):
        estimation, latent_params = self.forward(obs_history)
        z, v, p = estimation
        latent_mu, latent_var, vel_mu, vel_var, pos_mu, pos_var = latent_params 
        # Reconstruction loss
        recons = self.decode(z,vel)
        recons_loss =F.mse_loss(recons, next_obs,reduction='none').mean(-1)
        # Supervised loss
        v = torch.clamp(v.clone(), min=-1e5, max=1e5)
        vel = torch.clamp(vel.clone(), min = -1e5, max = 1e5)
        vel_loss = F.mse_loss(v, vel,reduction='none').mean(-1)

        p = torch.clamp(p.clone(), min=-1e5, max=1e5)
        pos = torch.clamp(pos.clone(), min=-1e5, max=1e5)
        pos_loss = F.mse_loss(p, pos,reduction='none').mean(-1)


        kld_loss = -0.5 * torch.sum(1 + latent_var - latent_mu ** 2 - latent_var.exp(), dim = 1)

        loss = recons_loss + velo_weight * vel_loss + kld_weight * kld_loss + pos_weight * pos_loss
        ...
        if (torch.isnan(vel_loss).any() | torch.isinf(vel_loss).any()):
            print('nan')
            vel_loss = torch.nan_to_num(vel_loss, nan=0.0, posinf=1e3, neginf=-1e3)
        if (torch.isnan(pos_loss).any() | torch.isinf(pos_loss).any()):
            print('nan')
            pos_loss = torch.nan_to_num(pos_loss, nan=0.0, posinf=1e3, neginf=-1e3)
    ...
    def inference(self,obs_history):
        ...
        latent_mu, latent_var, vel_mu, vel_var, pos_mu, pos_var = latent_params
        return [latent_mu, vel_mu, pos_mu]
