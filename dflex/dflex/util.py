...
def quat_from_axis_angle_batch(axis, angle):
    # Ensure axis is a numpy array
    axis = np.array(axis)

    # Compute half of the angle
    half_angle = angle * 0.5

    # Compute cosine and sine of half angle
    w = np.cos(half_angle)
    sin_theta_over_two = np.sin(half_angle)

    # Multiply the axis vectors by sin(half angle)
    v = axis * sin_theta_over_two

    # Stack the resulting vectors into quaternion form with 4 components (v[0], v[1], v[2], w)
    return np.hstack([v[:,0:3], np.full((axis.shape[0], 1), w)])
