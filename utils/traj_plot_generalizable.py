...
class TrajectoryPlanner:
    def __init__(self, ply_file, voxel_size=0.1, safety_distance=0.2, batch_size=1, wp_distance=2.0, verbose=True):
        ...
        self.points, self.colors = self.load_point_cloud()
    ...
    def visualize_trajectories(self, trajectory_batches=None, reference_trajectories=None):
        # pdb.set_trace()
        ...
        height_mask = (self.points[:,2] < 2.0) & (self.points[:,2] > 0.12) & \
                       (self.points[:,0] > -8.5) & (self.points[:,0] < 8.5) & \
                       (self.points[:,1] > -3.5) & (self.points[:,1] < 3.5)
        pcd.points = o3d.utility.Vector3dVector(self.points[height_mask])
        if hasattr(self, "colors") and self.colors is not None:
            filtered_colors = self.colors[height_mask]
        ...
        for idx, trajectory in enumerate(trajectory_batches):
            ...
            # pdb.set_trace()
            trajectory_points[:,2] = 1.35
            # trajectory_points = trajectory_points[:380]
            # trajectory_points[:,1] += 0.3
            trajectory_points = np.delete(trajectory_points, np.arange(120,200), axis=0)
            ...
            tube_mesh = create_tube(trajectory_points[:], radius=0.05, color=colors[0])
            geometries.append(tube_mesh)

        if reference_trajectories:
            for idx, trajectory in enumerate(reference_trajectories):
                if trajectory is None or len(trajectory) == 0:
                    if self.verbose:
                        print(f"Reference trajectory {idx+1} is empty or not found.")
                    continue

                trajectory_points = np.array(trajectory, dtype=np.float64)
                if trajectory_points.ndim != 2 or trajectory_points.shape[1] != 3:
                    if self.verbose:
                        print(f"Reference trajectory {idx+1} has incorrect shape: {trajectory_points.shape}")
                    continue

                color = colors[idx % len(colors)]
                ref_tube_mesh = create_tube(trajectory_points, radius=0.025, color=colors[0])
                geometries.append(ref_tube_mesh)
        ...
        if self.destination_positions is not None:
            ...
            dest = self.destination_positions[0]
        ...
        if self.current_positions is not None:
            current_spheres = []
            sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.3)
            dest = self.current_positions[0]
            sphere.translate(dest)
            color = colors[4]
            sphere.paint_uniform_color(color)
            current_spheres.append(sphere)
            geometries.extend(current_spheres)


        # Visualize all geometries
        o3d.visualization.draw_geometries(geometries)

    def get_real_trajectories(self, path):
        traj_x = np.load(os.path.join(path, 'x.npy'), allow_pickle=True)-6.0
        traj_y = np.load(os.path.join(path, 'y.npy'), allow_pickle=True)
        traj_z = np.load(os.path.join(path, 'z.npy'), allow_pickle=True)-0.2
        traj = np.stack([traj_x, traj_y, traj_z], axis=1).tolist()
        return [traj]
...
if __name__ == "__main__":
    ...
    # ## Generalizable gate
    # gate_mid
    ply_file = "/home/david/DiffRL_NeRF/envs/assets/point_cloud/sv_1007_gate_mid_color.ply"
    batch_size = 1
    current_positions_torch = torch.tensor([[-6.0, 0, 1.3]], device='cuda:0').repeat(batch_size,1)

    destination_positions_torch = torch.tensor([[2.0, -0.3, 1.3]], device='cuda:0').repeat(batch_size,1)
    waypoints_torch = [
                torch.tensor([
                              [-2.0, 0.0, 1.3],
                              [-0.2, -0.2, 1.4],
    ...
    destination_positions_torch = torch.tensor([[2.0, 0.5, 1.2]], device='cuda:0').repeat(batch_size,1)
    ...
    # batch_size = 1
    # current_positions_torch = torch.tensor([[-6.0, 0.3, 1.2]], device='cuda:0').repeat(batch_size,1)

    # destination_positions_torch = torch.tensor([[2.0, -1.0, 1.2]], device='cuda:0').repeat(batch_size,1)
    ...
    planner = TrajectoryPlanner(ply_file, batch_size=batch_size, safety_distance=0.1, wp_distance=2.0, verbose=False)
    start_t = time.time()
    ...
    # real_traj_path = '/home/david/DiffRL_NeRF/examples/outputs/quadrotor_gs_mask_pos_traj_global_multi_gate/gate_right/02-23-2025-16-50-01_paper_candidate'
    ...
    if trajectories:
        ...
        print(trajectories)
        # planner.visualize_trajectories(planner.trajectory_batches)
        # planner.visualize_trajectories(planner.get_real_trajectories(real_traj_path), planner.trajectory_batches)
        planner.visualize_trajectories(planner.get_real_trajectories(real_traj_path))
